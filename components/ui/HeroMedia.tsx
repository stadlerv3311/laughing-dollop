...
/**
 * Full-width photo or video filling the first screen under the fixed header. A warm sunset grade and
 * film grain sit on top, so a video dropped in later gets the same look (styles in app/globals.css).
 */
export function HeroMedia({ image, video }: HeroMediaProps) {
  return (
    <div className="pt-18">
      <div className="relative h-[62svh] min-h-80 w-full overflow-hidden bg-ink sm:h-[calc(100svh-4.5rem)]">
        <Image
          src={image.src}
          alt={image.alt}
          fill
        ...
        {video && (
          <video
            className="absolute inset-0 size-full object-cover saturate-[1.15] motion-reduce:hidden"
            autoPlay
            muted
            loop
            playsInline
