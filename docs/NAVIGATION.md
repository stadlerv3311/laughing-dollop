...
| `home/` | `SafetyBand` | GPS, dash cams and maintenance records — text and a hairline list left, photo right with an angled left edge; the rows play through on a 10 s timer (a line fills under the picked row; tap to jump) and each swaps the photo for its clip, on white; it sits straight under the numbers band, before Ship with us (swapped 2026-09-23) | Client (hover state) |
