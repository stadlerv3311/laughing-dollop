...
Header behavior (Lucid-inspired): stays in view while you scroll, so Apply To Drive and Get a Quote are always one click away (changed 2026-09-10 — it used to hide on scroll down on every screen). On phones (under 640px), where the bar only shows the logo and menu button, it still hides on scroll down and returns on scroll up. During the homepage intro the nav and buttons stay visible but dimmed over the scene and come to full on hover (requested 2026-09-10); the logo appears only once it lands from the intro. "Careers" opens a small panel with two links that drops from under the bar — a dropdown, not a mega-menu. Careers links: Drive For Us → `/careers/drivers`, Dispatch, Shop & Office → `/careers/staff`.
